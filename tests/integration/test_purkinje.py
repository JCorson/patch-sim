...
def test_pk_recovers_from_sustained_suprathreshold_drive(
    ...
    """Purkinje repolarises after +10 µA/cm² × 200 ms.

    Regression test for #324: the slow Na⁺ inactivation gate added to
    ``make_inap_channel`` lets the cell escape any depolarisation
    plateau seeded by sustained suprathreshold drive at the
    REPETITIVE_FIRING amplitude (10 µA/cm²).  The membrane settles
    below −50 mV during the last 200 ms of a 700 ms post-stimulus
    epoch (Purkinje is a spontaneous pacemaker, so the post-step
    epoch contains pacemaker spikes between AHPs around −65 mV).
    """
