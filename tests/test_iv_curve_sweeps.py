...
def test_batch_multi_sweep_matches_sequential() -> None:
    ...
    assert len(batch) == len(sequential)
    assert sequential[0].dtype.names is not None
    for seq_arr, batch_arr in zip(sequential, batch):
        assert seq_arr.dtype == batch_arr.dtype
        for field in seq_arr.dtype.names:
            np.testing.assert_array_equal(seq_arr[field], batch_arr[field])
