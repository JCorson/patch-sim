import pytest
from ap_sim.src.nernst_neuron import nernst_potential
...
def test_sodium():
    ...
    expected = 67.49  # Expected result in mV (approximate)
    result = nernst_potential(z, T, ion_concentration_out, ion_concentration_in)
    assert pytest.approx(result, 0.01) == expected
...
def test_calcium():
    ...
    expected = 120.69  # Expected result in mV (approximate)
    result = nernst_potential(z, T, ion_concentration_out, ion_concentration_in)
    assert pytest.approx(result, 0.01) == expected
