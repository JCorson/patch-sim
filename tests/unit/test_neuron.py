...
def test_calcium_current_accepts_e_rev_by_name() -> None:
    ...
    from patch_sim.channels import GatingVariable
    ...
    e_rev_by_name = {ch.name: ch.reversal_potential(neuron) for ch in neuron.all_channels}

    result_live = neuron.calcium_current(V=-40.0, gating_state=gs)
    result_cached = neuron.calcium_current(V=-40.0, gating_state=gs, e_rev_by_name=e_rev_by_name)
