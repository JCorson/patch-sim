...
def test_compute_dvdt_constant_voltage_returns_zero_dvdt() -> None:
    ...
    dvdt = compute_dvdt(time, voltage)
...
def test_compute_dvdt_output_length_matches_input() -> None:
    """Output array has the same length as the input arrays."""
...
def test_compute_dvdt_single_point_returns_empty_dvdt() -> None:
    """A single-point input returns an empty dvdt array."""
    time = np.array([0.0])
...
def test_compute_dvdt_two_points_returns_same_length() -> None:
    """A two-point input is the minimum for a derivative; output length matches."""
...
def test_compute_dvdt_empty_input_returns_empty() -> None:
    """Empty input arrays return an empty dvdt array."""
    dvdt = compute_dvdt(np.array([]), np.array([]))
    assert len(dvdt) == 0
...
def test_compute_dvdt_accepts_lists() -> None:
    ...
    dvdt = compute_dvdt([0.0, 1.0, 2.0], [0.0, 0.0, 0.0])
