...
def make_stn_na_channel(
    ...
    *,
    ...
    By default the channel uses two gates: ``"m"`` (activation, power 3) and
    ``"h"`` (fast inactivation, power 1), matching the original Otsuka 2004
    formulation.  With ``slow_inactivation=True`` a third gate ``"sNa"`` is
    added (V½ = −50 mV, slope 8 mV, inverted Boltzmann).  The slow gate is
    mostly available at hyperpolarised potentials and decays towards zero on
    sustained depolarisation, providing an escape mechanism that lets the
    membrane repolarise after a prolonged suprathreshold step (#324).  The
    simulation column is named ``sNa`` rather than ``s`` to avoid colliding
    with :func:`~patch_sim.additional_channels.make_inar_channel`'s
    activation gate.

    Slow inactivation is opt-in because it changes the effective Na⁺
    availability during sustained firing trains, which can alter F-I and
    spike-frequency-adaptation profiles in presets calibrated against the
    no-slow-inactivation Otsuka kinetics.  Enable it on presets where
    depolarisation-block recovery is the dominant concern — e.g. STN.
