...
NEURON_PRESETS: dict[str, NeuronConfig] = {
    ...
    STN: NeuronConfig(
        ...
        #
        # PARTIAL FIX — depolarization-block recovery (#324): INaP is opted
        # into the slow voltage-dependent inactivation gate (sNaP, Magistretti
        # & Alonso 1999) via ``slow_inactivation=True`` below, which removes
        # >70 % of the persistent Na⁺ window current during sustained
        # suprathreshold drive.  However, the Otsuka 2004 fast Na⁺ kinetics
        # retain a small h tail (h_inf ≈ 1 % at −15 mV) which, scaled by
        # g_Na = 30 mS/cm², still drives ~16 µA/cm² inward at the depol-block
        # plateau and can sustain a partial plateau under high drive
        # (≳ +5 µA/cm²).  Real STN cells also use ATP-K and K⁺-accumulation
        # mechanisms — none modelled.  The autonomous tonic phenotype
        # (≤ ~+2 µA/cm²) is unaffected.
        #
